...
def process_db_casia(db_dir, save_dir, scale, crop_sz):
    ...
    for file in open(db_dir+"/file_list.txt","r"):
        ...
        file_addr_name = file_name.split("\\")
        save_addr = os.path.join(cur_save_dir,file_addr_name[-4],file_addr_name[-3],file_addr_name[-2])
        if not os.path.exists(save_addr):
            os.makedirs(save_addr)
        save_fname = os.path.join(save_addr, file_addr_name[-1])
...
if __name__ == "__main__":
    ...
    db_dir = 'E:/mini/dataset/CASIA_frames'
    save_dir = 'E:/mini/dataset/CASIA_scale'
